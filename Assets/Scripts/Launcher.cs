...
namespace FourtyFourty
{
	...
	public class Launcher : MonoBehaviourPunCallbacks
    {
		...
		public void RestartTheGame()
		{
			...
			Debug.Log("RestartTheGame buttong pressed");
		}
		...
		public void ShowStats()
		{
            ...
            matchesWonTimeText.text =  ((PlayerPrefs.GetInt(UTIL.TOTAL_GAMES_WON , 0)*1.0f)/PlayerPrefs.GetInt(UTIL.TOTAL_GAMES_LOST , 0)).ToString("n2");
		...
		void Update()
		{
			...
			if(timer.IsTimeOut() && !timeOut && PhotonNetwork.IsConnected)
			{
				timeOut = true;
				gamePanelGameObj.SetActive(true);
		...
		void ShowConnectionStatus(string status , bool isActive){
			if(!CheckInternetConnection()){
				connctingStatusText.text = "Please check internet connection\n And retry";
				StartCoroutine(HideConnectingPanel(3));
			}else {
		...
		IEnumerator HideConnectingPanel(float dealy)
		{
			yield return new WaitForSeconds(dealy);
